...
public class anothery extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		for (i = 0; i < 10; i++) {
			for (j = 0; j < 10; j++) {
				if (con.a.count[i][yyy][j] != 0) {
					for (k = 0; k < con.a.count[i][yyy][j]; k++) {
						...
						tm = "x: "
								+ Integer
										.toString(con.a.suoyin[i][yyy][j][k * 3])
								+ "y: "
								+ Integer
										.toString(con.a.suoyin[i][yyy][j][k * 3 + 1])
								+ "z: "
								+ Integer
										.toString(con.a.suoyin[i][yyy][j][k * 3 + 2]);
						li.add(tm);
